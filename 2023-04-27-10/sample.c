...
int main(int argc, char *argv[]) {
    ...
    while(fgets(line, 102, fin)) {
        ...
        strcpy(airports[length].name,
            strtok(line, ";"));
        strcpy(airports[length].city,
            strtok(NULL, ";"));
        airports[length].runways = 
            atoi(strtok(NULL, ";"));
        airports[length].time = 
            atoi(strtok(NULL, ";"));
        length++;
    }
    ...
    if(!fout) {
        fprintf(stderr,
             "Cannot open 2nd file\n");
        return 4;
    }

    qsort(airports, length, sizeof(AIRPORT),
        cmp);
